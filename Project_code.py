...
def trie_liste_frag(liste):                                                 #fonction qui trie selon la taille (décroissant) des éléments comme migration 
    ...
    while(modif==True):                                                     #tant qu'on fait une modification on reparcourt la liste
        ...
        for i in range(len(liste)-1):
            if(len(liste[i+1])>len(liste[i])):                              #trie à bulles
...
def afficheResultDig(enz,liste):                                            #fonction pour afficher le résultat de digestion d'une ou plusieurs enzymes
    ...
    for i in range(len(liste)):                                             #parcourt liste de fragment pour l'afficher
...
def digestion(ENZ, seq,dico):                                               #fonction de digestion d'une séquence d'ADN par une seule enzyme
    var=dico[ENZ]                                                           #variable pour récupérer les informations du site de coupure de l'enzyme dans le dictionnaire (site et indice de coupure)
    var=var.split(",")                                                      #.split pour séparer les données récupérées du dico
    seq_siteR = var[0]                                                      
    ...
    for i in range(len(seq)-len(seq_siteR)+1):                              #incrémentation de 1 du début à fin séq ADN (on retire la taille du site de coupure pour ne pas être hors cadre de lecture à la fin)
        ...
        if (trouver_siteR(cadre,seq_siteR)):                                
            l_frag.append(seq[dernier_coupure:i+index_coupe])               #mise à jour de la liste des fragments
...
def digestion_multiple(l_enz,seq,dico):                                     #fonction pour gérer les digestions par plusieurs enzymes
    l_frag=[]                                                               #liste finale de tous les fragments digérés
    for i in range(len(l_enz)):                                             #parcourt la liste des enzymes choisies
        if (i==0):
            l_frag=digestion(l_enz[i],seq,dico)
        else:                               
            l_frag2=[]                                                      #liste intermédiaire des fragments générés par une des enzymes
...
##########################
def Menu():                                                                 #fonction principale qui va gérer les différents choix possibles du programme
    utilisation=True
    ...
    while(utilisation):                                                     #Boucle while permettant plusieurs choix à la suite
        choix=int(input("\n Quel est votre choix: "))                       #Demande du choix de l'utilisateur

        if (choix==1):                                                      #choix 1 pour afficher les enzymes du dictionnaire
            Afficher_Dico_Enz()
        ...
        elif(choix ==3):                                                    #choix 3 pour digérer une séquence par une seule enzyme
            dico_enz=creation_dico()                                        #création du dictionnaire
            seq=input("Entrez la séquence d'ADN à digérer: ")
            seq=seq.upper()                                                 #on met la séquence donnée en MAJUSCULE pour la traiter 
            enz=input("Entrez le nom de l'enzyme: ")
            if(verif_Enz(enz,dico_enz) and verif_seq(seq)):                 #vérifie la présence d'erreurs avant de lancer la digestion
                l_frag=digestion(enz,seq,dico_enz)
                afficheResultDig(enz,l_frag)
                
            else:                                                           #s'il existe au moins une erreur, affichage de messages personalisés selon l'erreur
        ...
        elif (choix == 4):                                                  #choix 4 pour la digestion de la séquence par plusieurs enzymes
            dico_enz=creation_dico()
            seq=input("Entrez la séquence d'ADN à digérer: ")
            seq=seq.upper()
            n=int(input("Entrez le nb enzymes pour digestion multiple : ")) #demande le nombre d'enzyme à entrer
            test_enz=True
            enz=[]                                                          #liste des enzyme données par l'utilisateur
            ...
            if(test_enz and test_seq):                                      #si aucune erreur, on procède à la digestion
                ...
                if(not(test_enz)):
                    print("!!! Erreur ",bug," n'est/ne sont pas dans le dictionnaire!!!")
                    print("Vérifiez l'écriture ou ajoutez la à partir du menu")
                if(not(test_seq)):
                    print("Donnez une séquence d'ADN pour faire une digestion")
        ...
        else:                                                               #Valeur ne correspondant à aucun des choix proposés
            print("Entrée non conforme aux choix")

Menu()                                                                      #lancement du programme
